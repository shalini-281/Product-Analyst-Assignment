"""Generate SOLUTION.md -- a concise response with links to the detailed artefacts.

The per-part READMEs carry the full working. This is the document a reviewer
reads end to end: the answer to each part, the evidence for it, and a pointer to
the detail. Kept deliberately short; the depth is one click away.

    .venv/bin/python Assignment_solution/common/build_solution_doc.py
"""
...
SOL = paths.SOLUTION_ROOT.name

DOC = f"""# Capillary — Product Analyst Assignment
...
.venv/bin/python {SOL}/part1/code/run_audit.py          # and part2, 6, 7, 8
```
...
### Part 1 — Data Quality & Governance Audit → [full write-up]({SOL}/part1/)
...
### Part 2 — Feature Engineering → [full write-up]({SOL}/part2/)
...
### Part 3 — Pipeline Design → [full write-up]({SOL}/part3/)
...
### Part 4 — Production Incident → [full write-up]({SOL}/part4/)
...
### Part 5 — Stakeholder Communication → [full write-up]({SOL}/part5/) · [the message]({SOL}/part5/slack_message.md)
...
### Part 6 — Which brand is most generous? → [full write-up]({SOL}/part6/)
...
### Part 7 — Win-back list → [full write-up]({SOL}/part7/) · [the 20 members]({SOL}/part7/output/wb_target_list.csv)
...
### Part 8 — Points liability → [full write-up]({SOL}/part8/)
...
def main() -> None:
    out = paths.DATA_ROOT / "SOLUTION.md"
    out.write_text(DOC)
    words = len(DOC.split())
    print(f"SOLUTION.md: {words:,} words, {len(DOC.splitlines()):,} lines -> {out}")
